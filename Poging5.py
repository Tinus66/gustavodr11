...
if menu == 'Wereldwijd':
    ...
    # Zorg ervoor dat de sliders de eerste unieke waarde gebruiken als standaard
    min_popularity = st.select_slider(
        'Kies een Populariteit:',
        options=sorted(df_global['Popularity'].unique()),
        value=sorted(df_global['Popularity'].unique())[0]  # Eerste waarde als standaard
    )
    min_danceability = st.select_slider(
        'Kies een Danceability:',
        options=sorted(df_global['Danceability'].unique()),
        value=sorted(df_global['Danceability'].unique())[0]  # Eerste waarde als standaard
    )
    min_acousticness = st.select_slider(
        'Kies een Acousticness:',
        options=sorted(df_global['Acousticness'].unique()),
        value=sorted(df_global['Acousticness'].unique())[0]  # Eerste waarde als standaard
    )

    # Genre dropdown met 'Alles' optie
    genres = ['Alles'] + list(df_global['Genre'].unique())
    selected_genre = st.selectbox('Selecteer een Genre', options=genres)

    # Filter de data op basis van de waarden
    ...
    # Alleen filteren op genre als er niet 'Alles' is geselecteerd
